...
- **4 new text-based games requested** (player feedback on the home page): "2 pure text and 2 half text half graphics on the screen like oregon trail." Both pure-text games shipped: `huntfox` (Hunt the Wumpus clone, randomly-generated cave) and `eggheist` (hand-authored choice-driven mystery, fixed map) — see their own HANDOFF.md files. Still open: two Oregon-Trail-style half-text-half-graphics games (status/stat panel alongside simple graphics, choice-driven events).
