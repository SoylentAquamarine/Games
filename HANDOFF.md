...
- **Per-game HANDOFF.md rollout in progress:** convention is `public/games/<slug>/HANDOFF.md` — see `docs/README.md`. 54 of 91 games have one as of this pass (started with the ones most recently touched); rolling out to the rest a few at a time per pass, same cadence as comment processing. When you finish work on a game, check whether it has a HANDOFF.md and update/create it before moving on. Games still missing one (mostly puzzle/card/board titles that haven't needed a feedback-driven pass yet): blackjack, board, candycrush, carddeck, chutes, colorswitch, dotsboxes, dropmerge, fibonacci, fifteen, floodit, fruitninja, gomoku, hanoi, helixjump, knifehit, lightsout, mancala, mastermind, maze, musicmaker, nonogram, pianotiles, reversi, rps, simon, sokoban, spritegen, stack, stratego, sudoku, threes, tictactoe, timberman, ultimatettt, whack, wordsearch.
- **Per-game admin config rollout in progress:** `admin/games/index.html`'s Configuration pane falls back to a raw-JSON textarea games don't actually read yet, for any game without a dedicated pane wired into `CFG_PANES`/`showConfig()`. Two patterns exist so far: board/level editors (quest, chickenmania, adventure, minigolf — visual grid/canvas editing) and a generic, data-driven numeric-knob table (`NUMERIC_CONFIGS` in `admin/games/index.html` — a handful of number inputs merged into each game's `C` object via an explicit allowlist at boot; add a game with one table entry + a matching allowlist snippet in that game's own file, no new admin HTML/JS). 14 games use the numeric-knob pattern so far: kaboom, centipede, boulderdash, qbert, duckhunt, missilecommand, galaga, digdug, bomberman, lander, moonpatrol, dino, joust, pinball — see kaboom's HANDOFF.md for the full explanation. It's much cheaper to add than a board editor and fits most games (anything with tunable difficulty constants in its `C` export) — prefer it unless the game genuinely has an editable board/level shape. Rolling out a game or two at a time, same cadence as HANDOFF.md docs. Many action/arcade games with tunable constants still lack an admin pane entirely — check `CFG_PANES`/`NUMERIC_CONFIGS` before assuming one exists.
