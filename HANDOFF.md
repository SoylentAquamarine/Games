...
- **`mascots.js`** — `window.Mascots.spacesuitChicken(ctx,x,y,r,facing)` (basic) and `window.Mascots.spacesuitChickenFlying(ctx,x,y,r,facing)` (jetpack + life-support-pack variant, for shootable flybys): the site-wide decorative spacesuit-chicken cameo (bonus catches, flybys). Started as a per-game copy-pasted sprite — already drifting (missing details in some copies) before this library existed. Now used by duckhunt, lander, kaboom, dk, chickenmania, boulderdash, centipede, missilecommand, qbert, and drmario's bonus round (10 games total). Player feedback asked for a proper mascot library (Hero chicken, rooster, Dr Chicken/surgical-mask variant too) — only the spacesuit-chicken part is built so far. **Still open:** moonpatrol's cameo is a simpler sprite that doesn't match either library variant closely enough to swap in silently (needs a design call — upgrade it to match, or add a "mini" variant); drmario's *other* spacesuit chicken (the persistent "Dr Chicken" side-panel portrait) is deliberately unmigrated — the player wants that one redesigned as a surgical-mask character, not unified with the spacesuit look; and the Hero chicken / rooster characters from the original comment haven't been started.
