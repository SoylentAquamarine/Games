...
- **Board/level editor rollout in progress (new initiative, distinct from the numeric-knob one below):** user request: "we need board editors on everything, and on adventure I need an individual board editor not just how to organize the boards." Scope agreed with the user: every game with a genuine grid/room/level concept, not literally every game (reflex/arcade games like snake or pong have no board to edit). 5 games have a real board/level editor so far: `quest` (paint floor/wall/rock per dungeon room), `chickenmania` (edit wave formations), `minigolf` (draw wall/sand/water per hole, multi-course), `adventure` (drag rooms on the position grid and draw/erase each room's interior walls), and `sokoban` (newly added — paint wall/floor/target/box/player per level on a fixed grid, add/delete levels; the 4 built-in levels are preserved exactly). Next candidates, roughly in order of how cheap they'd be to wire up:
