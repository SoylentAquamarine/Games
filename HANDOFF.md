...
- **4 new text-based games requested** (player feedback on the home page): "2 pure text and 2 half text half graphics on the screen like oregon trail." First one shipped: `huntfox` (Hunt the Wumpus clone, pure text, no canvas — see its own HANDOFF.md). Still open: one more pure-text game, and two Oregon-Trail-style half-text-half-graphics games (status/stat panel alongside simple graphics, choice-driven events).
