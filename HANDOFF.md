...
- **Per-game admin config rollout in progress:** `admin/games/index.html`'s Configuration pane falls back to a raw-JSON textarea games don't actually read yet, for any game without a dedicated pane wired into `CFG_PANES`/`showConfig()`. Two patterns exist so far: board/level editors (see the rollout note directly above) and a generic, data-driven numeric-knob table (`NUMERIC_CONFIGS` in `admin/games/index.html` — a handful of number inputs merged into each game's `C` object via an explicit allowlist at boot; add a game with one table entry + a matching allowlist snippet in that game's own file, no new admin HTML/JS). 25 games use the numeric-knob pattern so far: kaboom, centipede, boulderdash, qbert, duckhunt, missilecommand, galaga, digdug, bomberman, lander, moonpatrol, dino, joust, pinball, chixxon, chickencircus, colorswitch, pianotiles, timberman, fruitninja, helixjump, knifehit, chickenposition, pool, drmario (newest — `C` previously only held board-shape constants, but `virusesFor`/`dropSpeed` already had a real, once-player-tuned difficulty curve as inline formula literals; extracted the 6 coefficients into `C` and rewired the formulas to read them, same as `crossyroad` would need before it can be wired up too) — see kaboom's HANDOFF.md for the full explanation. It's much cheaper to add than a board editor and fits most games (anything with tunable difficulty constants in its `C` export) — prefer it unless the game genuinely has an editable board/level shape. Rolling out a game or two at a time, same cadence as HANDOFF.md docs. Remaining candidates: `chicken1` (has a rich per-race config already, similar to chickenposition — would need the same care picking only independent knobs), `crossyroad` (its difficulty constants — lane speed/spawn density — are inline `Math.random()` calls, not in a `C` object at all, so it needs the same "extract into named constants first" treatment drmario just got). Neither is a cheap "just add a table entry" win like the ones already done — treat them as their own small design decisions, not a mechanical batch. Many other games (snake, pacman, invaders, asteroids, breakout, frogger, flappy, doodlejump, pong, tron, whack, bubbleshooter, and most puzzle/card/board titles) predate the `C`-object convention and use plain local variables instead — adding config to those would mean refactoring the game's internals first, a bigger job than the "one table entry + one allowlist snippet" cost that makes this rollout cheap elsewhere; don't do that opportunistically, only as its own deliberate pass.
