...
- **Per-game admin config rollout — numeric-knob pattern done.** `admin/games/index.html`'s Configuration pane falls back to a raw-JSON textarea games don't actually read yet, for any game without a dedicated pane wired into `CFG_PANES`/`showConfig()`. Two patterns exist: board/level editors (see the rollout note directly above — still has open candidates) and a generic, data-driven numeric-knob table (`NUMERIC_CONFIGS` in `admin/games/index.html` — a handful of number inputs merged into each game's `C` object via an explicit allowlist at boot; add a game with one table entry + a matching allowlist snippet in that game's own file, no new admin HTML/JS). 27 games use the numeric-knob pattern: kaboom, centipede, boulderdash, qbert, duckhunt, missilecommand, galaga, digdug, bomberman, lander, moonpatrol, dino, joust, pinball, chixxon, chickencircus, colorswitch, pianotiles, timberman, fruitninja, helixjump, knifehit, chickenposition, pool, drmario, chicken1, crossyroad (last one — its road/water chance and car/log speed ranges were inline `Math.random()` literals inside `genRow()`, not fields on `C`; extracted 8 coefficients into `C` and rewired `genRow()` to read them, defaults verified unchanged by comparing the generated row-type distribution over 20k rows before/after) — see kaboom's HANDOFF.md for the full explanation. Every game with a `const C={...}` block and genuine, independent difficulty constants now has a config pane; the remaining ~unwired games (snake, pacman, invaders, asteroids, breakout, frogger, flappy, doodlejump, pong, tron, whack, bubbleshooter, and most puzzle/card/board titles) predate the `C`-object convention and use plain local variables instead — adding config there means refactoring the game's internals first, a bigger job than the "one table entry + one allowlist snippet" cost that made this rollout cheap; only take one on as its own deliberate pass, not opportunistically.
