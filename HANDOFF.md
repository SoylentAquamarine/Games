...
- **Board/level editor rollout in progress (new initiative, distinct from the numeric-knob one below):** user request: "we need board editors on everything, and on adventure I need an individual board editor not just how to organize the boards." Scope agreed with the user: every game with a genuine grid/room/level concept, not literally every game (reflex/arcade games like snake or pong have no board to edit). 4 games have a real board/level editor so far: `quest` (paint floor/wall/rock per dungeon room), `chickenmania` (edit wave formations), `minigolf` (draw wall/sand/water per hole, multi-course), and `adventure` (drag rooms on the position grid AND — newly added this pass — draw/erase each room's interior walls; previously the admin editor only handled room position/colour, not the actual room layout, which was the specific gap in the user's request). Next candidates, roughly in order of how cheap they'd be to wire up:
  - `sokoban` — already has a fixed `LEVELS` array (4 hand-authored levels) in the game file itself, the same shape quest/minigolf's editors already expect (an array of levels to pick from and edit) — likely the cheapest next win.
  - `pacman` — has a single fixed `MAP` (the maze layout), no multi-level concept, closer to adventure's "one editable layout" shape than quest's "N levels" shape.
